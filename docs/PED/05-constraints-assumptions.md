...
| CN-03 | Cost / resources | Free or low-cost services only. Free-tier limits and likely operational cost beyond the educational context must be identified. | Rules out paid messaging gateways (CF-03), managed object storage at scale (SCOPE-D-01) and always-on compute. Free tiers idle out and cold-start, so: quality targets are set against a warm instance (NFR-001) with cold start declared as a known limitation (NFR-002); each separately deployed service is another instance that idles out, pushing M2 architecture towards a single deployable unit; M3 load testing must separate warm from cold measurements; and NFR-010 states operational cost at 100 users / 1 000 requests per month. Residual exposure is RSK-02. The chain is worked through in §5.1. |
